...
export function ExhibitBuilder({
  ...
  const toggleDocument = useCallback((doc: DocumentItem) => {
  ...
  const updateLabel = useCallback((index: number, label: string) => {
    setSelectedDocs((prev) =>
      prev.map((d, i) => (i === index ? { ...d, exhibitLabel: label } : d)),
    );
  }, []);
  ...
  }, [caseId, title, description, selectedDocs, onPacketCreated]);
      ...
      <Sheet open={open} onOpenChange={handleOpenChange}>
        ...
        >
          <SheetHeader>
            ...
            <SheetDescription>
              Select documents and arrange them into an exhibit packet for
              submission.
            </SheetDescription>
          </SheetHeader>
          ...
          <div className="flex-1 overflow-y-auto space-y-4 py-4">
            ...
            <div className="space-y-2">
              <Label>Select Documents ({selectedDocs.length} selected)</Label>
            ...
            {buildStatus && (
              <div className="rounded-md bg-muted p-3">
                <p className="text-sm text-foreground">{buildStatus}</p>
              </div>
            )}
          </div>
          ...
          <SheetFooter className="border-t pt-4">
            ...
            <Button
              onClick={handleBuild}
              disabled={isPending || !title.trim() || selectedDocs.length === 0}
            >
