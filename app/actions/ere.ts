...
export async function createEreCredential(data: {
	...
	const [credential] = await db
		...
		.values({
			...
			totpSecretEncrypted: data.totpSecret
...
export async function submitEreScrapeJob(data: {
	caseId: string;
	credentialId: string;
	jobType?: "full_scrape" | "incremental_sync" | "document_download" | "status_check";
}) {
	const session = await requireSession();
	...
	try {
		...
		logger.warn("ERE integration not available, job created but not dispatched", {
			jobId: job.id,
			error: err,
		});
	}
...
export async function testEreCredential(credentialId: string) {
	...
	try {
		...
		if (!isConfigured()) {
			...
			revalidatePath("/admin/integrations/ere");
			return { success: true, message: "Credential saved. Scraper service not available for live verification." };
		}
