...
/**
 * Full list of pipeline statuses the DB enum accepts. Keep in sync with
 * `leadStatusEnum` in `db/schema/enums.ts` and the migration that adds new
 * values.
 */
 ...
 * layout and the `getLeadStatusCategory` helper below. Keep the ordering in
 * sync with `leadStatusEnum` in `db/schema/enums.ts`.
 */
 ...
 * The page/client can use these to tint pills and column headers.
 */
