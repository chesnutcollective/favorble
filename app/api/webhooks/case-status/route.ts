...
async function resolveCaseId(
	...
	return caseRow ? { caseId: caseRow.id, organizationId: caseRow.organizationId } : null;
}
...
export async function POST(request: NextRequest) {
	try {
		...
		try {
			...
			if (isDev) {
				logger.warn("Case Status signature verification error (ignoring in dev)", {
					error: verifyError,
				});
			} else {
		...
		switch (eventType) {
			...
			case "document.uploaded": {
				...
				const fileType =
					body.fileType ?? body.contentType ?? body.mimeType ?? "application/octet-stream";
				...
				await db.insert(documents).values({
					...
					storagePath: body.downloadUrl ?? body.url ?? `pending/case-status/${body.documentId ?? body.id ?? "unknown"}`,
					source: "case_status",
