  ...
  if (stageGroups.length === 0) {
    try {
      ...
      if (connStr) {
        ...
        const groups =
          await sql`SELECT * FROM case_stage_groups ORDER BY display_order ASC`;
        const stages =
          await sql`SELECT * FROM case_stages WHERE deleted_at IS NULL ORDER BY display_order ASC`;
        await sql.end();
