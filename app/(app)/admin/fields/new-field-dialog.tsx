		...
		startTransition(async () => {
			try {
				await createFieldDefinition({
					...
					formula: fieldType === "calculated" ? formula.trim() || undefined : undefined,
				});
					...
					<div className="space-y-4 py-4">
						...
						{fieldType === "calculated" && (
							<div className="space-y-2">
								...
								/>
								<p className="text-xs text-muted-foreground">
									Use {"{field_slug}"} to reference other
									fields. Supports arithmetic (+, -, *, /),
									IF(condition, then, else), AGE(date),
									ROUND, ABS, MIN, MAX.
								</p>
							</div>
						)}
