...
export function ChronologyClient({
	...
	const [entries, setEntries] =
		useState<ChronologyEntryItem[]>(initialEntries);
	const [packets, setPackets] =
		useState<ExhibitPacketItem[]>(initialPackets);
	...
	const [editingEntry, setEditingEntry] =
		useState<ChronologyEntryItem | null>(null);
	...
	const handleVerify = useCallback(
		(entryId: string, verified: boolean) => {
			startTransition(async () => {
				try {
					if (verified) {
	...
	const handleExclude = useCallback((entryId: string) => {
		startTransition(async () => {
			try {
				...
				try {
					...
					setEntries((prev) => [
						...prev,
						{
							...
							eventDateEnd:
								entry.eventDateEnd?.toISOString() ?? null,
					...
					editingEntry
						? {
								...
								eventDate: editingEntry.eventDate
									...
									: "",
								providerName:
									editingEntry.providerName ?? "",
								facilityName:
									editingEntry.facilityName ?? "",
								summary: editingEntry.summary,
								details: editingEntry.details ?? "",
								diagnoses:
									editingEntry.diagnoses ?? [],
								treatments:
									editingEntry.treatments ?? [],
								medications:
									editingEntry.medications ?? [],
							}
						: undefined
				}
			/>
