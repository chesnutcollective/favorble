...
function getFieldOptions(field: FieldDefinition): string[] {
	...
	if (Array.isArray(field.options)) {
		return field.options.map((o) =>
			typeof o === "string" ? o : (o as { label?: string; value?: string })?.value ?? String(o),
		);
	}
	return [];
}
...
export function EditableFieldsForm({
	...
	function isValueEmpty(
		...
		switch (fieldType) {
			...
			case "multi_select":
				return !val.jsonValue || (Array.isArray(val.jsonValue) && val.jsonValue.length === 0);
			default:
			...
			case "text":
				return (
					<Input
						...
						onChange={(e) =>
							onChange({ textValue: e.target.value || null })
						}
