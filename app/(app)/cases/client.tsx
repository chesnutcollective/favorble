...
import { createCase, bulkChangeCaseStage, assignStaffToCase } from "@/app/actions/cases";
...
export function CasesListClient({
	...
	function handleExportCsv() {
		...
		const headers = ["Case Number", "Claimant", "Stage", "Assigned To", "Last Updated"];
		const csvRows = rows.map((c) => [
			c.caseNumber,
			c.claimant ? `${c.claimant.lastName}, ${c.claimant.firstName}` : "Unknown",
			c.stageName ?? "",
						...
						<DialogContent>
							...
							<div className="space-y-4 py-2">
								...
								<div className="space-y-1.5">
									...
									<Select value={ncStageId} onValueChange={setNcStageId}>
										...
										<SelectContent>
											{(initialStages.length > 0 ? initialStages : stages).map((s) => (
							...
							<DialogFooter>
								...
								<Button
									...
									disabled={!ncFirstName || !ncLastName || !ncStageId || isPending}
								>
					...
					<Dialog open={bulkStageOpen} onOpenChange={setBulkStageOpen}>
						...
						<DialogContent>
							<DialogHeader>
								...
								<DialogDescription>
									Move {selectedIds.size} case{selectedIds.size > 1 ? "s" : ""} to a new stage.
								</DialogDescription>
							</DialogHeader>
							<div className="py-4">
