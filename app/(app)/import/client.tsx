...
export function ImportWizard() {
					...
					(header) => {
						...
						if (!match) {
							if (
								...
								match = "address";
							else if (normalized.includes("zip"))
								match = "zip";
							else if (
								normalized === "st" ||
								normalized === "state"
	...
	const handleGoToPreview = useCallback(async () => {
		const hasFirstName = mappings.some(
			(m) => m.caseFlowField === "firstName",
		);
		const hasLastName = mappings.some(
						...
						>
							...
							<div className="space-y-2">
								...
								<p className="text-xs text-muted-foreground">
									Accepts .csv files with headers in the first
									row
								</p>
							</div>
						</div>
				...
				<Card>
					...
					<CardContent>
						<p className="text-sm text-muted-foreground mb-4">
							Match each CSV column to the corresponding CaseFlow
							field. First Name and Last Name are required.
						</p>
							...
							{mappings.map((mapping) => (
								...
								>
									...
									<div className="w-1/2">
										<Select
											value={
												mapping.caseFlowField || "__skip__"
											}
											onValueChange={(val) =>
												handleMappingChange(
													mapping.csvColumn,
						...
						{!hasRequiredMappings && (
							<p className="text-sm text-destructive mt-4">
								You must map at least First Name and Last Name
								to proceed.
							</p>
						)}
						...
						<div className="flex justify-between mt-6">
							...
							<Button
								...
								disabled={
									!hasRequiredMappings || detectingDuplicates
								}
							>
								{detectingDuplicates
									? "Checking duplicates..."
					...
					<CardContent>
						{duplicates.length > 0 && (
							<div className="rounded-md bg-destructive/10 p-3 mb-4">
								...
								<p className="text-xs text-muted-foreground mt-1">
									The following rows match existing cases and
								...
								<ul className="text-sm mt-2 space-y-1">
									{duplicates.map((d) => (
										<li key={d.rowIndex}>
											Row {d.rowIndex + 1}:{" "}
											{d.firstName} {d.lastName} matches
						...
						<div className="flex justify-between mt-6">
							...
							<Button onClick={handleImport}>
								Import{" "}
								{rows.length - duplicates.length} Cases
							</Button>
						</div>
					</CardContent>
				</Card>
