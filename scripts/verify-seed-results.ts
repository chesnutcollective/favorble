...
async function main() {
	...
	const ev = await sql`SELECT event_type, COUNT(*) FROM calendar_events GROUP BY event_type`;
	console.log(ev);

	console.log("\n=== cases with hearing_date ===");
	const hd = await sql`SELECT COUNT(*) FROM cases WHERE hearing_date IS NOT NULL`;
	console.log(hd);

	console.log("\n=== cases with hearing_office set ===");
	const ho = await sql`SELECT COUNT(*) FROM cases WHERE hearing_office IS NOT NULL`;
	console.log(ho);
	...
	const om = await sql`SELECT mail_type, COUNT(*) FROM outbound_mail GROUP BY mail_type`;
	console.log(om);
	...
	const rfc = await sql`SELECT status, COUNT(*) FROM rfc_requests GROUP BY status`;
	console.log(rfc);
	...
	const al = await sql`SELECT entity_type, COUNT(*) FROM audit_log GROUP BY entity_type`;
	console.log(al);
	...
	const at = await sql`SELECT application_type_primary, COUNT(*) FROM cases GROUP BY application_type_primary`;
	console.log(at);
