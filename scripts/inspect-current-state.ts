...
async function main() {
	...
	const hdc = await sql`SELECT COUNT(*) FROM cases WHERE hearing_date IS NOT NULL`;
	console.log(hdc);

	console.log("\n=== cases with ssa_claim_number count ===");
	const ssnc = await sql`SELECT COUNT(*) FROM cases WHERE ssa_claim_number IS NOT NULL`;
	console.log(ssnc);

	console.log("\n=== calendar_events (by type) ===");
	const ce = await sql`SELECT event_type, COUNT(*) FROM calendar_events GROUP BY event_type`;
	console.log(ce);
	...
	const docs = await sql`SELECT tags, COUNT(*) FROM documents GROUP BY tags LIMIT 10`;
	console.log(docs);

	console.log("\n=== contacts sample ===");
	const contacts = await sql`SELECT id, type, first_name, last_name, organization_name FROM contacts LIMIT 5`;
