...
function fakeEncrypted(label: string): string {
	return Buffer.from(`seed-placeholder::${label}::${faker.string.nanoid(16)}`).toString("base64");
}
...
const PIPELINE_STAGES: PipelineStage[] = [
	...
	{ id: "web_form_submitted", label: "Web Form Submitted", group: "NEW_LEADS", order: 2 },
	{ id: "phone_call_received", label: "Phone Call Received", group: "NEW_LEADS", order: 3 },
	{ id: "walk_in", label: "Walk-In", group: "NEW_LEADS", order: 4 },
	{ id: "referral_received", label: "Referral Received", group: "NEW_LEADS", order: 5 },
	{ id: "marketing_lead", label: "Marketing Lead", group: "NEW_LEADS", order: 6 },
	// QUALIFICATION
	{ id: "initial_qualifying", label: "Initial Qualifying", group: "QUALIFICATION", order: 10 },
	{ id: "call_attempted_1", label: "Call Attempted 1", group: "QUALIFICATION", order: 11 },
	{ id: "call_attempted_2", label: "Call Attempted 2", group: "QUALIFICATION", order: 12 },
	{ id: "call_attempted_3", label: "Call Attempted 3", group: "QUALIFICATION", order: 13 },
	{ id: "voicemail_left", label: "Voicemail Left", group: "QUALIFICATION", order: 14 },
	{ id: "no_answer", label: "No Answer", group: "QUALIFICATION", order: 15 },
	{ id: "wrong_number", label: "Wrong Number", group: "QUALIFICATION", order: 16 },
	{ id: "intake_scheduled", label: "Intake Scheduled", group: "QUALIFICATION", order: 17 },
	{ id: "intake_rescheduled", label: "Intake Rescheduled", group: "QUALIFICATION", order: 18 },
	// INTAKE
	{ id: "intake_in_progress", label: "Intake In Progress", group: "INTAKE", order: 20 },
	{ id: "intake_complete", label: "Intake Complete", group: "INTAKE", order: 21 },
	{ id: "awaiting_documents", label: "Awaiting Documents", group: "INTAKE", order: 22 },
	{ id: "documents_received", label: "Documents Received", group: "INTAKE", order: 23 },
	{ id: "conflict_check_pending", label: "Conflict Check Pending", group: "INTAKE", order: 24 },
	{ id: "conflict_check_cleared", label: "Conflict Check Cleared", group: "INTAKE", order: 25 },
	// DECISION
	{ id: "contract_sent", label: "Contract Sent", group: "DECISION", order: 30 },
	{ id: "contract_signed", label: "Contract Signed", group: "DECISION", order: 31 },
	{ id: "retainer_paid", label: "Retainer Paid", group: "DECISION", order: 32 },
	{ id: "declined_by_firm", label: "Declined By Firm", group: "DECISION", order: 33 },
	{ id: "declined_by_client", label: "Declined By Client", group: "DECISION", order: 34 },
	{ id: "could_not_reach", label: "Could Not Reach", group: "DECISION", order: 35 },
	// CONVERSION
	{ id: "converting_to_case", label: "Converting to Case", group: "CONVERSION", order: 40 },
	...
	{ id: "spanish_routed", label: "Spanish Routed", group: "CONVERSION", order: 44 },
...
async function main() {
	...
	if (!org) throw new Error("No organization found. Run seed-demo-data.ts first.");
	const orgId: string = org.id;

	const users = await sql`SELECT id, email, first_name, last_name, role FROM users`;
	const attorneys = users.filter((u) => u.role === "attorney" || u.role === "admin");
	...
	`;
	console.log(`Loaded: org=${orgId}, users=${users.length}, cases=${cases.length}`);
	...
	for (let i = 0; i < cases.length; i++) {
		...
		const secondary = hasSecondary ? pick(APP_TYPES.filter((t) => t !== appType)) : null;
		...
		const dli = faker.datatype.boolean(0.7)
			? new Date(aod.getTime() + faker.number.int({ min: 365 * 3, max: 365 * 5 }) * 86400000)
			: null;
	...
	if (Number(existingHearings[0].n) < 20) {
		...
		const newHearingRows: Array<{ caseId: string; startAt: Date; alj: string; office: string }> = [];
		for (let i = 0; i < 30; i++) {
	...
	console.log(`  ✓ Updated ${existingLeadsNoStage.length} existing leads with pipeline stage`);

	// Create 50 new leads distributed across stages.
	const leadCountBefore = await sql`SELECT COUNT(*) as n FROM leads WHERE organization_id = ${orgId}`;
	...
	if (needNewLeads) {
		for (let i = 0; i < 50; i++) {
			...
			const source = pick(["website", "referral", "google_ads", "facebook", "walk_in", "phone"]);
			const lastContacted =
				Math.random() < 0.7 ? daysAgo(faker.number.int({ min: 0, max: createdDaysAgo })) : null;
		...
		console.log(`  ↺ Skipped new lead creation (already ${leadCountBefore[0].n} leads)`);
	}
	...
	const omCount = await sql`SELECT COUNT(*) as n FROM outbound_mail WHERE organization_id = ${orgId}`;
	if (Number(omCount[0].n) < 20) {
		...
		for (let i = 0; i < 25; i++) {
			...
			const deliveredAt = delivered
				? new Date(sentAt.getTime() + faker.number.int({ min: 1, max: 5 }) * 86400000)
				: null;
			const tracking =
				mailType === "certified"
	...
	if (Number(mailDocsCount[0].n) < 10) {
		const mailSubjects = [
			{ fname: "ssa_notice_of_hearing.pdf", desc: "SSA Notice of Hearing", category: "ssa_notice" },
			{ fname: "medical_records_request.pdf", desc: "Medical records from provider", category: "medical" },
			{ fname: "ssa_decision_letter.pdf", desc: "SSA Denial Letter", category: "ssa_decision" },
			{ fname: "appeal_acknowledgment.pdf", desc: "SSA Appeal Acknowledgment", category: "ssa_notice" },
			{ fname: "doctor_letter.pdf", desc: "Treating physician letter", category: "medical" },
			{ fname: "return_receipt.pdf", desc: "USPS return receipt card", category: "correspondence" },
			{ fname: "workers_comp_records.pdf", desc: "Workers comp file", category: "employment" },
			{ fname: "ssa_claim_status.pdf", desc: "Claim status update from SSA", category: "ssa_notice" },
			{ fname: "mri_report.pdf", desc: "Lumbar MRI report", category: "medical" },
			{ fname: "psychiatric_eval.pdf", desc: "Psych consultative exam", category: "medical" },
		];
		for (let i = 0; i < 15; i++) {
			const c = pick(cases);
		...
		console.log(`  ✓ Created 15 inbound mail documents (tagged 'mail', pending_processing)`);
	} else {
	...
	const pcCount = await sql`SELECT COUNT(*) as n FROM provider_credentials WHERE organization_id = ${orgId}`;
	if (Number(pcCount[0].n) === 0) {
		for (const p of PATIENT_PORTAL_PROVIDERS) {
			const lastUsed = Math.random() < 0.8 ? daysAgo(faker.number.int({ min: 0, max: 30 })) : null;
			await sql`
	...
	const rfcCount = await sql`SELECT COUNT(*) as n FROM rfc_requests WHERE organization_id = ${orgId}`;
	if (Number(rfcCount[0].n) === 0) {
		const statusDist: Array<"not_requested" | "requested" | "received" | "completed"> = [
			"not_requested",
			"not_requested",
			"requested",
			"requested",
		...
		for (let i = 0; i < 12; i++) {
			...
			const requestedAt = status !== "not_requested" ? daysAgo(faker.number.int({ min: 7, max: 60 })) : null;
			const receivedAt =
				status === "received" || status === "completed"
					? daysAgo(faker.number.int({ min: 0, max: 20 }))
					: null;
			const completedAt = status === "completed" ? daysAgo(faker.number.int({ min: 0, max: 10 })) : null;
			const dueDate = status !== "completed" ? daysFromNow(faker.number.int({ min: 3, max: 30 })) : null;
	...
	// =======================================================================
	console.log("\n--- Seeding audit_log ---");
	const auditTemplates: Array<{ entity: string; action: string; changes: any }> = [
		{ entity: "case", action: "viewed_phi", changes: { reason: "routine case review" } },
		{ entity: "case", action: "status_changed", changes: { from: "active", to: "active" } },
		{ entity: "case", action: "stage_advanced", changes: {} },
		{ entity: "case", action: "assigned", changes: {} },
		{ entity: "case", action: "field_updated", changes: { field: "hearing_date" } },
		...
		{ entity: "provider_credential", action: "used", changes: { purpose: "medical records download" } },
	];
	for (let i = 0; i < 50; i++) {
	...
	for (let i = 0; i < 30; i++) {
		...
		const clientEmail = claimant?.email ?? `${faker.person.firstName().toLowerCase()}@example.com`;
	...
	const ereCredCount = await sql`SELECT COUNT(*) as n FROM ere_credentials WHERE organization_id = ${orgId}`;
	...
	if (Number(ereCredCount[0].n) === 0) {
		...
		const existingEre = await sql`SELECT id FROM ere_credentials WHERE organization_id = ${orgId}`;
		ereCredIds = existingEre.map((r) => r.id);
		console.log(`  ↺ Skipped ere_credentials (already ${ereCredIds.length} rows)`);
	}

	const ereJobCount = await sql`SELECT COUNT(*) as n FROM ere_jobs WHERE organization_id = ${orgId}`;
	if (Number(ereJobCount[0].n) === 0 && ereCredIds.length > 0) {
		const jobStatuses: Array<"pending" | "running" | "completed" | "completed" | "completed" | "failed" | "cancelled"> = [
			"pending",
			"running",
			"completed",
			"completed",
		...
		const jobTypes: Array<"full_scrape" | "incremental_sync" | "document_download" | "status_check"> = [
			"full_scrape",
			"incremental_sync",
			"document_download",
			"status_check",
		...
		for (let i = 0; i < 20; i++) {
			...
			const startedAt = status === "pending" ? null : daysAgo(faker.number.int({ min: 0, max: 15 }));
			const completedAt =
				status === "completed" || status === "failed" || status === "cancelled"
					? startedAt
						? new Date(startedAt.getTime() + faker.number.int({ min: 60, max: 900 }) * 1000)
						: null
					: null;
			const docsFound =
				status === "completed" || status === "failed" ? faker.number.int({ min: 0, max: 120 }) : null;
			const docsDownloaded =
				status === "completed" ? docsFound : status === "failed" ? Math.floor((docsFound ?? 0) / 2) : null;
