  ...
  try {
    ...
    try {
      const statusLink = page
        .getByRole("link", { name: /status report/i })
      ...
      return {
        success: false,
        error: `Failed to navigate to Status Reports: ${error instanceof Error ? error.message : "unknown"}`,
      };
    }
    ...
    try {
      ...
      const downloadButton = page
        .getByRole("button", { name: /download spreadsheet/i })
      ...
      return {
        success: false,
        error: `Download failed: ${error instanceof Error ? error.message : "unknown"}`,
      };
    }
  } catch (error) {
