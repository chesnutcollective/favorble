...
export async function checkPickupFiles(
  page: Page,
  ssnLast4?: string,
  ...
  try {
    ...
    try {
      const pickupLink = page
        .getByRole("link", { name: /pick up files|pickup/i })
      ...
      return {
        success: false,
        error: `Failed to navigate to Pick Up Files: ${error instanceof Error ? error.message : "unknown"}`,
      };
    }
    ...
    if (ssnLast4) {
      console.log(
        `Filtered to ${files.length} files matching SSN ending in ${ssnLast4}`,
      );
    }
    ...
    const readyFiles = files.filter(
      (f) =>
        f.status.toLowerCase().includes("ready") ||
        f.status.toLowerCase().includes("complete"),
    );
  ...
  try {
    const files = await page.evaluate(() => {
      ...
      const rows = document.querySelectorAll(
        "table tr, .pickup-row, [class*='file-row']",
      );
      for (const row of rows) {
        ...
        if (cells.length >= 2) {
          ...
          const ssnMatch =
            text.match(/(\d{4})\s*$/m) || text.match(/\*{3}-\*{2}-(\d{4})/);
          ...
          result.push({
            ...
            status:
              ...
              "",
            dateReady:
              cells.length >= 3 ? cells[2]?.textContent?.trim() : undefined,
          });
        }
      }
    ...
    return files.filter(
      (f) =>
        f.fileName &&
        !f.fileName.toLowerCase().includes("file name") &&
        !f.fileName.toLowerCase().includes("filename"),
    );
  } catch (error) {
...
async function downloadPickupFile(
  page: Page,
  file: PickupFile,
): Promise<string | null> {
  try {
    ...
    const downloadLink = page
      .getByRole("link", { name: new RegExp(escapeRegex(file.fileName), "i") })
