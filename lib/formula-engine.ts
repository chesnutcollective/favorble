...
function tokenize(formula: string): Token[] {
  ...
  while (i < formula.length) {
    ...
    if (
      /\d/.test(ch) ||
      (ch === "." && i + 1 < formula.length && /\d/.test(formula[i + 1]))
    ) {
      let num = "";
      while (
        i < formula.length &&
...
function evaluate(
  ...
  switch (node.kind) {
    ...
    case "binary": {
      ...
      const leftNum =
        typeof left === "number" ? left : parseFloat(String(left)) || 0;
      const rightNum =
        typeof right === "number" ? right : parseFloat(String(right)) || 0;
