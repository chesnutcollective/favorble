...
export async function getIntegrationsStatus(): Promise<IntegrationsStatus> {
  ...
  const railwayService = (
    ...
    const meta: Array<{ label: string; value: string }> = [
      {
        label: "STATUS",
        value: rs?.deploymentStatus.toLowerCase() ?? "unknown",
      },
      {
        label: "DEPLOY",
        value: formatRelativeTime(rs?.lastDeployedAt ?? null),
      },
      ...(options.extraMeta ?? []),
    ];
    const health =
  ...
  if (
    n8nWarn &&
    n8nServiceIndex >= 0 &&
    services[n8nServiceIndex].status === "ok"
  ) {
  ...
  services.push({
    ...
    health: caseStatusConfigured ? "API key set" : "Pending real API key",
    action: { label: "Setup" },
  });
