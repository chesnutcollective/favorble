...
export async function processDocument({
  ...
  try {
    ...
    const encounterDate = findExtraction(result.extractions, "encounter_date");
...
export async function loadDocumentText(
  ...
  if (storagePath.startsWith("data:")) {
    try {
      ...
      if (isPdf) {
        const buffer = Buffer.from(payload, isBase64 ? "base64" : "utf-8");
...
async function createChronologyEntry(params: {
  ...
  await db.insert(medicalChronologyEntries).values({
    ...
    details: extractions
      .map((e) => `${e.extraction_class}: ${e.extraction_text}`)
      .join("\n"),
