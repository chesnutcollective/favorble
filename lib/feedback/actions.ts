...
export async function buildClaudeExportAction(input?: {
  ...
  const prompt = [
    ...
    "",
    "Each item with a `screenshot.url` field has a pixel-perfect JPEG screenshot of the page the user was viewing. The URL returns a real image file with `Content-Type: image/jpeg`, signed with the same 48h API token.",
    "",
    "**If you're in Claude Code (recommended):**",
    "```bash",
    `# For each item you're triaging:`,
    `curl -sL "<screenshot.url>" -o /tmp/feedback-<id>.jpg`,
    "```",
    "Then use the `Read` tool on `/tmp/feedback-<id>.jpg` — Claude Code processes image files as vision input automatically.",
    "",
    "**If you're on claude.ai (web):** open the URL in your browser, then drag the image into your chat.",
    "",
    "## API",
    `- Base: ${baseUrl}`,
