...
export async function buildClaudeExportAction(input?: {
  ...
  const prompt = [
    ...
    "",
    "5. **Implement the fix**, then update the item's status via the API below (`building` when starting work, `staging` when deployed to staging branch, `production` when merged to main). In your PR description or status `notes`, mention which other locations you also updated so the triager can verify coverage.",
    "",
